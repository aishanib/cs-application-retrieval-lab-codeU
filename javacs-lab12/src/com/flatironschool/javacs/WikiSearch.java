...
public class WikiSearch {
	...
	public WikiSearch and(WikiSearch that) {
        // FILL THIS IN!
				Map<String, Integer> intersection = new HashMap<String, Integer>(map);
				for(String term: map.keySet()){
					int relevance = 0;
					if(that.map.containsKey(term)){
						relevance = totalRelevance(this.getRelevance(term), that.getRelevance(term));
					}
	...
	public WikiSearch minus(WikiSearch that) {
        Map<String, Integer> difference = new HashMap<String, Integer>(map);
				for(String term: map.keySet()){
					int relevance = 0;
					if(!that.map.containsKey(term)) relevance = this.getRelevance(term);
					difference.put(term, relevance);
				}
	...
	public List<Entry<String, Integer>> sort() {
        List<Entry<String,Integer>> sorted = new LinkedList<Entry<String, Integer>>(map.entrySet());
